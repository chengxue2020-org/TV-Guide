...
# ==================== 浏览器模拟配置 ====================
USE_CURL = True                     # 是否使用 curl 命令下载
CURL_USER_AGENT = 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0.0.0 Safari/537.36'
CURL_IMPERSONATE = 'chrome120'      # curl_cffi 风格的浏览器指纹（如果安装）
USE_COOKIE_PERSISTENCE = True       # 是否保存和复用 Cookie
...
def download_with_curl(url: str, output_path: str, referer: str = None) -> bool:
    """
    使用 curl 命令下载文件
    ...
    if USE_COOKIE_PERSISTENCE and os.path.exists(cookie_file):
        ...
        cmd.extend(['--cookie-jar', cookie_file])
    else:
        cmd.extend(['--cookie-jar', cookie_file])
    
    # 添加请求头
    cmd.extend(['--header', 'Accept: text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,image/apng,*/*;q=0.8'])
    cmd.extend(['--header', 'Accept-Language: zh-CN,zh;q=0.9,en;q=0.8'])
    ...
    try:
        print(f'    🔧 curl 命令: curl -L --max-time {DOWNLOAD_TIMEOUT} ... {url}')
        result = subprocess.run(cmd, capture_output=True, text=True, timeout=DOWNLOAD_TIMEOUT + 10)
        
        if result.returncode == 0 and os.path.exists(output_path) and os.path.getsize(output_path) > 0:
            return True
        else:
            if result.stderr:
                print(f'    ⚠ curl 错误: {result.stderr[:200]}')
            return False
...
def download_with_python(url: str, path: str, referer: str = None) -> Optional[str]:
    """使用 Python requests 下载（备用方案）"""
    ...
    try:
        ...
        try:
            ...
            response = curl_requests.get(url, headers=headers, impersonate=CURL_IMPERSONATE, timeout=DOWNLOAD_TIMEOUT, allow_redirects=True)
        except ImportError:
            import requests
...
def download_file(url: str, path: str) -> Optional[str]:
    ...
    for attempt in range(MAX_RETRIES + 1):
        ...
        if USE_CURL:
            ...
            if download_with_curl(url, temp_path, referer):
                # 检查文件是否为有效的 XML 或 GZIP
                with open(temp_path, 'rb') as f:
                    content = f.read(100)
                if content.startswith(b'<?xml') or content.startswith(b'<tv') or content.startswith(b'\x1f\x8b'):
                    os.rename(temp_path, download_path)
                    file_size = os.path.getsize(download_path)
                    print(f'    ✓ curl 下载成功: {format_size(file_size)}')
                    success = True
                else:
                    print(f'    ⚠ curl 下载的内容不是有效 XML')
                    os.remove(temp_path)
        
        # 备用：使用 Python 下载
        if not success:
            result = download_with_python(url, path, referer)
            if result:
                # 检查文件
                ...
                if content.startswith(b'<?xml') or content.startswith(b'<tv') or content.startswith(b'\x1f\x8b'):
                    ...
                    if result != download_path:
                        os.rename(result, download_path)
                    file_size = os.path.getsize(download_path)
                    print(f'    ✓ Python 下载成功: {format_size(file_size)}')
                    success = True
                else:
                    print(f'    ⚠ Python 下载的内容不是有效 XML')
                    os.remove(result)
...
def main() -> None:
    ...
    print_separator('=')
    print('Guide Merger v2.0 (curl + Python Hybrid)')
    print_separator('=')
    ...
    print(f'✓ 智能合并: {SMART_MERGE}')
    print(f'✓ 下载方式: {"curl" if USE_CURL else "Python requests"}')
    print()
